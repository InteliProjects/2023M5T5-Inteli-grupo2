...
import { getAlgorithm, getAllPathsAlgorithm, useApiNodesTools } from "../../services/nodes";
import { getNodes, getEdges } from "../../services/nodes.ts"
...
export default function Graph() {
  ...
  const rpn = location.pathname.split('/').pop();
  console.log(rpn);
  const [ cookies ] = useCookies(['authToken']);
  const authToken = cookies.authToken;
  const api= useApiNodesTools();

  // addAuthTokenToApiRequestNodes(api, authToken);
  ...
  const handleMinAlgo = async () => {
    try {
      ...
      let allNodes = [];
      allNodes = await getNodes(authToken);
      ...
      let ourNodes: [] = [];
        ...
        for (let i = 0; i < allNodes.length; i++) {
          for (let j = 0; j < allNodeNames[n].length; j++) {
            if (allNodes[i]["name"] == allNodeNames[n][j] && allNodes[i]["rpn"] == rpn) {
              ourNodes.push(allNodes[i]);
            }
          }
        }
  ...
  let loadNodes = async () => {
    try {
      ...
      let allEdges = [];
      allNodes = await getNodes(authToken);
      allEdges = await getEdges(authToken);
